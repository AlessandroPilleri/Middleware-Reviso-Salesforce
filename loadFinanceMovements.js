...
function getRevisoClients(n, callback) {
    ...
    var options = {
        url: 'https://rest.reviso.com/report-api/v1/finance-movements?skippages=' + n + '&pagesize=200',
        headers: {
...
function formatClients(clients, callback) {
    ...
    console.log(obj)
    clients.lines.forEach(function (d, i) {
        var field = {}
        console.log(d)
...
function doUpsert(objs, conn, callback) {
    while (objs.length > 0) {
        ...
        console.log(o)
        conn.sobject("Movimenti_contabili__c").upsert(o, 'Account_Number__c', function (err, rets) {
            console.log('ciao')
            if (err) {
                console.log(err)
                callback(err)
